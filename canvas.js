...
(function () {
  ...
  CanvasEdit = function (layercontainer, overlaycontainer, width, height) {
    ...
    this.stratchpad = makeDrawspace_(width, height, 0);
    this.sandbox = makeDrawspace_(width, height, 1);
    this.mask = makeDrawspace_(width, height, 2);
    ...
    layercontainer.appendChild(firstLayer.layer.canvas());
    overlaycontainer.appendChild(this.stratchpad.layer.canvas());
    overlaycontainer.appendChild(this.sandbox.layer.canvas());
    overlaycontainer.appendChild(this.mask.layer.canvas());
    return this;
  }
  ...
  method.addLayer = function () {
    ...
    this.container.appendChild(entry.layer.canvas());
    return entry;
  };
...
//    this.getLayer(index).style.hidden = 
  ...
  method.selectLayer = function (node) {
    //if (this.nodes.find())
    this.activeLayer[0] = node.layer;
  };
